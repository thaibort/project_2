...
<div class="w-full h-screen bg-red-200 m-2">
        ...
        <form action="{{url('admin/upschyear')}}" method="post" class="flex">
            ...
            @method("PUT")
            @foreach($rs as $item)
                <input type="text" name="id" value="{{$item->id}}" hidden required class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1" tabindex="-1">
                <label class="col-4 flex mr-xl-5"> Niên khóa: <br>
                    <input type="number" value="{{$item->name}}" name="name" required class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1" tabindex="-1">
                </label>
                <label class="col-4"> Đợt đóng tiền hiện tại: <br>
                    <input type="number" value="{{$item->stagesPresent}}" name="stagesPresent" required class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1" tabindex="-1">
                </label>
                @endforeach
