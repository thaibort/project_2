...
<div class="w-full h-screen bg-red-200 m-2">
    <div class="pt-2">
        <a href="{{url('admin/scholarship')}}"><button aria-colspan="3" type="button"
    ...
    <div class="pt-2">
        <form action="{{url('admin/upscholarship')}}" method="post" class="flex">
            ...
            @method("PUT")
            @foreach($rs as $res)
            <input type="text" name="id" value="{{$res->id}}" hidden required
            ...
            <label class="col-4 flex mr-xl-5">Học Bổng Loại <br>
                <input type="number" value="{{$res->type}}" name="name" required
                    class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1"
                    tabindex="-1">
            </label>
            <label class="col-4">Số Tiền <br>
                <input type="number" value="{{$res->money}}" name="money" required
                    class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1"
                    tabindex="-1">
            </label>
            @endforeach
